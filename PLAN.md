...
Breaking changes and patterns from the bundled docs that affect our build:

### `params` and `searchParams` are now Promises
In Next.js 15, page/layout props `params` and `searchParams` are `Promise` objects. Must `await` them in Server Components or use `React.use()` in Client Components:
```ts
// Server Component
...
All components are Server Components unless marked `'use client'`. Our visualizer, audio engine, controls, and code editor are all client-side — keep the `'use client'` boundary at the component level, not the page level. Algorithm pages should be Server Components that render SEO text, then compose in client visualizer components.

### Tailwind CSS v4 style
The docs show the new Tailwind PostCSS plugin and `@import` syntax:
```css
/* globals.css */
@import 'tailwindcss';
```

### Metadata is Server Component only
`export const metadata` and `generateMetadata()` can only be used in Server Components (layouts/pages). This works fine — our algorithm pages are server-rendered.
...
- Rich metadata in `layout.tsx` via the Next.js `metadata` export:
  ...
  - `robots.ts` and `sitemap.ts` generated via Next.js file conventions (not route handlers)
